...
def create_arithmetic_counterfactual_dataset(seed : int = 42, size : int = 10000):
    rng = np.random.RandomState(seed)
    X_base = rng.uniform(-50, 50, size=(size, 2))  # Each row is a sample: [x, y] # I should change "X" to "points"
    y_base = -X_base[:, 0] - X_base[:, 1]

    X_source = rng.uniform(-50, 50, size=(size, 2))  # Each row is a sample: [x, y] # I should change "X" to "points"
    y_source = -X_source[:, 0] - X_source[:, 1]

    y_counterfactual_x = -X_source[:, 0] - X_base[:, 1]
    y_counterfactual_y = -X_base[:, 0] - X_source[:, 1]

    return (
        torch.tensor(X_base).float(), 
        torch.tensor(X_source).float(), 
        torch.tensor(y_base).float(), 
        torch.tensor(y_source).float(), 
        torch.tensor(y_counterfactual_x).float(),
        torch.tensor(y_counterfactual_y).float()
    )
